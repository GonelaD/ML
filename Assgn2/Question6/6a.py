...
def random_forests(data,min_size,n_features):
    ...
    X=[[0 for y in range(len(training[0])-1)]for x in range(len(training))]
    y=[0 for x in range(len(training))]
    for i in range(len(training)):
    	for j in range(len(training[0])-1):
    		X[i][j] = training[i][j]
    	y[i] = training[i][len(training[0])-1]

    clf.fit(X,y)

    X_test=[[0 for y in range(len(testing[0])-1)]for x in range(len(testing))]
    y_test=[0 for x in range(len(testing))]
    for i in range(len(testing)):
    	for j in range(len(testing[0])-1):
    		X_test[i][j] = testing[i][j]
    	y_test[i] = testing[i][len(testing[0])-1]

    print "Accuracy using :",clf.score(X_test,y_test)




		result = forest.classifyOOB(instance[:-1], i, sample_bag)
...
if __name__ == "__main__":
    ...
    n_features = 57
    random_forests(data,min_size,n_features)

...
def random_forests(data,min_size,n_features):
    ...
    for i in range(len(training)):
    	...
    	for j in range(tree_number):
    		for element in sample[j]:
    			if np.array_equal(element, instance):
    				flag=True
    		if flag == False:
    			result.append(predict(trees[j],instance))
    		else:
    			present.append(j)

    	if not result:
    		predictions.append(bagging_and_predict(trees,instance))
    	else:
    		predictions.append(max(set(result),key=result.count))

    for i in range(len(training)):
    	if training[i][len(training[0])-1]==predictions[i]:
    		results.append(True)
    	else:
    		results.append(False)

    accuracy = float(results.count(True))/float(len(results))
    print "OOB Train Error :",1-accuracy
